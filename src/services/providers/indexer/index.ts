import type { API, Application, Donation, Project, Round, Transformers } from "../../types";
import {
  roundsQuery,
  applicationsQuery,
  roundsByIdQuery,
...
import { AnyVariables, Client, DocumentInput, cacheExchange, fetchExchange } from "@urql/core";
...
export const indexer: API["indexer"] = {
  rounds: async (query) => {
    return indexerQuery<{ rounds: GSRound[] }>(roundsQuery, queryToFilter(query)).then((res) =>
  ...
  applicationById: (id, opts) => {
    ...
    }).then((res) => (res?.application ? transformers.application(res.application) : undefined));
  },
  projects: (query) => {
    return indexerQuery<{ projects: GSProject[] }>(projectsQuery, queryToFilter(query)).then(
      (res) => (res?.projects ?? []).map(transformers.project),
    );
  },
  projectById: (id, opts) => {
    ...
    ).then((res) => (res?.projects?.[0] ? transformers.project(res.projects?.[0]) : undefined));
  },
  donations: (query) => {
    return indexerQuery<{ donations: GSDonation[] }>(donationsQuery, queryToFilter(query)).then(
      (res) => (res?.donations ?? []).map(transformers.donation),
    );
  },
};
...
export const transformers: Transformers<GSRound, GSApplication, GSProject, GSDonation> = {
  round: ({
    id,
    chainId,
    roundMetadata,
  ...
  project: ({ id, chainId, name, anchorAddress, metadata }: GSProject): Project => ({
    id,
    chainId,
    name,
    anchorAddress,
