...
export const allo: API["allo"] = {
  createRound: async function (data, signer) {
    ...
    return createLogDecoder(AlloABI, client)(hash!, ["PoolCreated"]).then((logs) => {
  ...
  createApplication: async function (data, signer) {
    ...
    return createLogDecoder(AlloABI, client)(hash!, ["UpdatedRegistration"]).then((logs) => {
      const id = String((logs?.[0]?.args as { recipientId: Address })?.recipientId);
      return { id, chainId: signer.chain?.id as number };
    });
  },
  ...
  getProfile: async function (signer) {
    ...
    const profile = await registry?.getProfileById(getProfileId(signer.account?.address!));
  ...
  createProfile: async function ({ metadata, name }, signer) {
    ...
    return createLogDecoder(AlloABI, signer.extend(publicActions))(hash, ["ProfileCreated"]).then(
      (logs) => {
...
*/
  sendTransaction: async function (tx: TransactionInput, signer?: WalletClient) {
    if (!signer?.account) throw new Error("Signer missing");
    const address = getAddress(signer.account?.address);
...
export function getProfileId(address: Address): Address {
  return keccak256(encodePacked(["uint256", "address"], [PROFILE_NONCE, address]));
}
